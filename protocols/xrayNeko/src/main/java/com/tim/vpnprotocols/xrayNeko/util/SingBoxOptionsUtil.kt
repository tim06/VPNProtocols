...
fun generateRuleSet(context: Context, ruleSetString: List<String>, ruleSet: MutableList<SingBoxOptions.RuleSet>) {
    ruleSetString.forEach {
        when {
            it.startsWith("geoip") -> {
                val geoipPath = GeoipUtils.generateRuleSet(context = context, country = it.removePrefix("geoip:"))
                ruleSet.add(SingBoxOptions.RuleSet().apply {
                    type = "local"
                    tag = it
                    format = "binary"
                    path = geoipPath
            ...
            it.startsWith("geosite") -> {
                val geositePath = GeositeUtils.generateRuleSet(context = context, code = it.removePrefix("geosite:"))
                ruleSet.add(SingBoxOptions.RuleSet().apply {
                    type = "local"
                    tag = it
                    format = "binary"
