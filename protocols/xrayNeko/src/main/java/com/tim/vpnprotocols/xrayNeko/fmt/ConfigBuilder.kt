...
import com.tim.vpnprotocols.xrayNeko.parser.DataStore
import com.tim.vpnprotocols.xrayNeko.parser.IPv6Mode
import com.tim.vpnprotocols.xrayNeko.util.JavaUtil.gson
import com.tim.vpnprotocols.xrayNeko.naive.Plugins
import com.tim.vpnprotocols.xrayNeko.fmt.ProxyEntity.Companion.TYPE_CONFIG
import com.tim.vpnprotocols.xrayNeko.fmt.SingBoxOptions.Rule_DefaultOptions
import com.tim.vpnprotocols.xrayNeko.util.SingBoxOptionsUtil
import com.tim.vpnprotocols.xrayNeko.parser.TunImplementation
import com.tim.vpnprotocols.xrayNeko.util.Util
import com.tim.vpnprotocols.xrayNeko.XRayNekoService
import com.tim.vpnprotocols.xrayNeko.util.checkEmpty
import com.tim.vpnprotocols.xrayNeko.fmt.hysteria.HysteriaBean
import com.tim.vpnprotocols.xrayNeko.fmt.hysteria.buildSingBoxOutboundHysteriaBean
...
import com.tim.vpnprotocols.xrayNeko.fmt.shadowsocks.buildSingBoxOutboundShadowsocksBean
import com.tim.vpnprotocols.xrayNeko.fmt.shadowtls.ShadowTLSBean
import com.tim.vpnprotocols.xrayNeko.fmt.shadowtls.buildSingBoxOutboundShadowTLSBean
import com.tim.vpnprotocols.xrayNeko.fmt.socks.SOCKSBean
import com.tim.vpnprotocols.xrayNeko.fmt.socks.buildSingBoxOutboundSocksBean
import com.tim.vpnprotocols.xrayNeko.fmt.ssh.SSHBean
import com.tim.vpnprotocols.xrayNeko.fmt.ssh.buildSingBoxOutboundSSHBean
import com.tim.vpnprotocols.xrayNeko.fmt.tuic.TuicBean
import com.tim.vpnprotocols.xrayNeko.fmt.tuic.buildSingBoxOutboundTuicBean
import com.tim.vpnprotocols.xrayNeko.fmt.v2ray.StandardV2RayBean
import com.tim.vpnprotocols.xrayNeko.fmt.v2ray.buildSingBoxOutboundStandardV2RayBean
import com.tim.vpnprotocols.xrayNeko.fmt.wireguard.WireGuardBean
import com.tim.vpnprotocols.xrayNeko.fmt.wireguard.buildSingBoxOutboundWireguardBean
import com.tim.vpnprotocols.xrayNeko.util.generateRuleSet
import com.tim.vpnprotocols.xrayNeko.util.isIpAddress
import com.tim.vpnprotocols.xrayNeko.util.makeSingBoxRule
import com.tim.vpnprotocols.xrayNeko.util.mkPort
...
fun buildConfig(
    ...
    val group = null //SagerDatabase.groupDao.getById(proxy.groupId)
    ...
    val extraRules: List<RuleEntity> = listOf<RuleEntity>()//if (forTest) listOf() else SagerDatabase.rulesDao.enabledRules()
    val extraProxies: Map<Long, ProxyEntity> = emptyMap()/*if (forTest) {
        mapOf()
    } else {
    ...
    val buildSelector = false//!forTest && group?.isSelector == true && !forExport
    val userDNSRuleList = mutableListOf<SingBoxOptions.DNSRule_DefaultOptions>()
    val domainListDNSDirectForce = mutableListOf<String>()
    val bypassDNSBeans = hashSetOf<AbstractBean>()
    val isVPN = true // DataStore.serviceMode == Key.MODE_VPN
    ...
    return SingBoxOptions.MyOptions().apply {
        if (!forTest && DataStore.enableClashAPI) experimental = SingBoxOptions.ExperimentalOptions()
            .apply {
            ...
            cache_file = SingBoxOptions.CacheFile().apply {
                enabled = true
                store_fakeip = true
                path = "../cache/clash.db"
            }
        ...
        log = SingBoxOptions.LogOptions().apply {
        ...
        dns = SingBoxOptions.DNSOptions().apply {
            // TODO nb4a hosts?
...
//                .toMutableMap()
        ...
        if (!forTest) {
            if (isVPN) inbounds.add(SingBoxOptions.Inbound_TunOptions().apply {
                type = "tun"
                tag = "tun-in"
                ...
                when (ipv6Mode) {
                    IPv6Mode.DISABLE -> {
                        inet4_address = listOf(XRayNekoService.PRIVATE_VLAN4_CLIENT + "/28")
                    }

                    IPv6Mode.ONLY -> {
                        inet6_address = listOf(XRayNekoService.PRIVATE_VLAN6_CLIENT + "/126")
                    }
            ...
            inbounds.add(SingBoxOptions.Inbound_MixedOptions().apply {
                type = "mixed"
                tag = TAG_MIXED
                listen = bind
        ...
        fun buildChain(
            ...
            profileList.forEachIndexed { index, proxyEntity ->
                ...
                if (proxyEntity.needExternal()) { // externel outbound
                    ...
                    currentOutbound = SingBoxOptions.Outbound_SocksOptions().apply {
                        type = "socks"
                        server = LOCALHOST
...
//                        val needKeepAliveInterval = keepAliveInterval !in intArrayOf(0, 15)
                        ...
                        if (!muxApplied && proxyEntity.needCoreMux()) {
                            muxApplied = true
                            currentOutbound["multiplex"] = SingBoxOptions.MultiplexOptions().apply {
                                enabled = true
                ...
                if (bean.canMapping() && proxyEntity.needExternal()) {
                    ...
                    if (needExternal) {
                        ...
                        inbounds.add(SingBoxOptions.Inbound_DirectOptions().apply {
                            type = "direct"
                            listen = LOCALHOST
        ...
        if (buildSelector) {
            ...
            outbounds.add(0, SingBoxOptions.Outbound_SelectorOptions().apply {
                type = "selector"
                tag = TAG_PROXY
        ...
        for (rule in extraRules) {
            ...
            val ruleObj = Rule_DefaultOptions().apply {
                ...
                if (rule_set != null) generateRuleSet(context, rule_set, ruleSets)
                ...
                /*fun makeDnsRuleObj(): SingBoxOptions.DNSRule_DefaultOptions {
                    return SingBoxOptions.DNSRule_DefaultOptions().apply {
        ...
        for (freedom in arrayOf(TAG_DIRECT, TAG_BYPASS)) outbounds.add(SingBoxOptions.Outbound().apply {
            tag = freedom
            type = "direct"
        }.asMap())

        outbounds.add(SingBoxOptions.Outbound().apply {
            tag = TAG_BLOCK
            type = "block"
        }.asMap())

        if (!forTest) {
            inbounds.add(0, SingBoxOptions.Inbound_DirectOptions().apply {
                type = "direct"
                tag = TAG_DNS_IN
                listen = bind
        ...
        remoteDns.firstOrNull().let {
            dns.servers.add(SingBoxOptions.DNSServerOptions().apply {
        ...
        if (forTest) {
            ...
            dns.servers = listOf(
                SingBoxOptions.DNSServerOptions().apply {
                    address = LOCAL_DNS_SERVER
                    tag = "dns-local"
                    detour = TAG_DIRECT
                }
            ...
            // FakeDNS obj
            if (useFakeDns) {
                dns.fakeip = SingBoxOptions.DNSFakeIPOptions().apply {
                    enabled = true
                ...
                dns.rules.add(SingBoxOptions.DNSRule_DefaultOptions().apply {
                    inbound = listOf("tun-in")
                    server = "dns-fake"
                    disable_cache = true
                })
            ...
            if (domainListDNSDirectForce.isNotEmpty()) {
                dns.rules.add(0, SingBoxOptions.DNSRule_DefaultOptions().apply {
