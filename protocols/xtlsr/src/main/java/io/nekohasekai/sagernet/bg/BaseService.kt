...
class BaseService {
    ...
    class Binder(private var data: Data? = null) : IVPNService.Stub(), CoroutineScope,
        AutoCloseable {
        private val callbacks = object : RemoteCallbackList<IConnectionStateListener>() {
            override fun onCallbackDied(callback: IConnectionStateListener?, cookie: Any?) {
                super.onCallbackDied(callback, cookie)
            }
        }

        val callbackIdMap = mutableListOf<IConnectionStateListener>()

        override val coroutineContext = Dispatchers.Main.immediate + Job()

        override fun getState(): ConnectionState = (data?.state ?: State.Idle).toConnectionState()
        ...
        suspend fun broadcast(work: (IConnectionStateListener) -> Unit) {
        ...
        fun stateChanged(s: State, msg: String?) = launch {
            ...
            broadcast { it.stateChanged(s.toConnectionState()) }
        }
    ...
    interface Interface {
        ...
        fun startVPN(config: ProxyEntity): Int {
            this as Context

            val profile = config //SagerDatabase.proxyDao.getById(DataStore.selectedProxy)
            if (profile == null) { // gracefully shutdown: https://stackoverflow.com/q/47337857/2245107
                data.notification = createNotification("")
                stopRunner(false, getString(R.string.profile_empty))
                return Service.START_NOT_STICKY
...
fun BaseService.State.toConnectionState(): ConnectionState {
    return when (this) {
        BaseService.State.Connected -> ConnectionState.CONNECTED
        BaseService.State.Connecting -> ConnectionState.CONNECTING
        BaseService.State.Stopped -> ConnectionState.DISCONNECTED
        BaseService.State.Stopping -> ConnectionState.DISCONNECTING
        BaseService.State.Idle -> ConnectionState.IDLE
    }
}