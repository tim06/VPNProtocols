				...
				try
				{
					...
					else
					{
											...
											(mCurrentProfile.getFlags() & VpnProfile.FLAGS_IPv6_TRANSPORT) != 0))
						{
							...
							writer.setValue("global.crl", false/*(mCurrentProfile.getFlags() & VpnProfile.FLAGS_DISABLE_CRL) == 0*/);
							writer.setValue("global.ocsp", false/*(mCurrentProfile.getFlags() & VpnProfile.FLAGS_DISABLE_OCSP) == 0*/);
							...
							writer.setValue("connection.certreq", false/*(mCurrentProfile.getFlags() & VpnProfile.FLAGS_SUPPRESS_CERT_REQS) == 0*/);
		...
		if (mService.notificationClassName != null) {
			...
			else
			{
				...
				switch (state)
				{
					case CONNECTING:
						if (vpnNotification != null) {
							vpnNotification.createNotification(getString(R.string.state_connecting));
						}
