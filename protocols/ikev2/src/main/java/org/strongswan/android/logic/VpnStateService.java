...
import com.tim.ikev2.BuildConfig;
import com.tim.ikev2.Ikev2Configuration;
import com.tim.ikev2.R;
...
public class VpnStateService extends Service {
    ...
    private final RemoteCallbackList<IConnectionStateListener> callbackList = new RemoteCallbackList<IConnectionStateListener>();
    ...
    private final DisconnectTimer disconnectTimer = new DisconnectTimer(
            s -> {
                for (VpnStateListener listener : mListeners) {
                    listener.notificationDescriptionChanged(s);
                }
                return null;
            },
            () -> {
     ...
     * after binding to it.
     */
    public class LocalBinder extends IVPNService.Stub {
        ...
        public void startVPN(VpnConfiguration configuration) throws RemoteException {
            ...
            if (configuration.getTimeToDisconnect() != null) {
                disconnectTimer.start(configuration.getTimeToDisconnect());
            }
            Ikev2Configuration ikev2Configuration = (Ikev2Configuration) configuration.getData();
            VpnProfile conf = new VpnProfile();
            conf.setUUID(UUID.fromString("f000aa01-0451-4000-b000-000000000000"));
            conf.setName(ikev2Configuration.getName());
            conf.setGateway(ikev2Configuration.getHost());
            conf.setUsername(((Ikev2Configuration) configuration.getData()).getLogin());
            conf.setPassword(((Ikev2Configuration) configuration.getData()).getPassword());
            conf.setVpnType(VpnType.IKEV2_EAP);

            TreeSet<String> apps = new TreeSet<String>(configuration.getAllowedApps());
            conf.setSelectedApps(apps);
            conf.setSelectedAppsHandling(VpnProfile.SelectedAppsHandling.SELECTED_APPS_ONLY);

            Bundle profileInfo = new Bundle();
            profileInfo.putString(
                    VpnProfileDataSource.KEY_UUID,
                    conf.getUUID().toString()
            );
            profileInfo.putString(VpnProfileDataSource.KEY_USERNAME, conf.getUsername());
            profileInfo.putString(VpnProfileDataSource.KEY_PASSWORD, conf.getPassword());
            profileInfo.putBoolean(
                    VpnProfileControlActivity.PROFILE_REQUIRES_PASSWORD,
                    conf.getVpnType().has(VpnType.VpnTypeFeature.USER_PASS)
            );
        ...
        @Override
        public ConnectionState getState() throws RemoteException {
            return convertState(VpnStateService.this.getState());
        }

        @Override
        public void registerCallback(IConnectionStateListener cb) throws RemoteException {
            callbackList.register(cb);
        }

        @Override
        public void unregisterCallback(IConnectionStateListener cb) throws RemoteException {
    ...
    public void reconnect() {
        ...
        if (mProfile.getVpnType().has(VpnType.VpnTypeFeature.USER_PASS)) {
            if (mProfile.getPassword() == null ||
                    mError == ErrorState.AUTH_FAILED) {    /* show a dialog if we either don't have the password or if it might be the wrong
                    ...
                    if (change.call()) {    /* otherwise there is no need to notify the listeners */
                        ...
                        new Handler(Looper.getMainLooper()).post(() -> {
                            if (callbackList.getRegisteredCallbackCount() > 0) {
                                int callbacks = callbackList.beginBroadcast();
                                for (int b = 0; b < callbacks; b++) {
                                    try {
                                        callbackList.getBroadcastItem(b).stateChanged(convertState(getState()));
                                    } catch (Exception e) {
                                        if (BuildConfig.DEBUG) {
                                            Log.e("State", "$exception: " + e);
                                        }
                                    }
                                }
    ...
    private void setRetryTimer(ErrorState error) {
        mRetryTimeout = 0;//mRetryIn = mTimeoutProvider.getTimeout(error);
        if (mRetryTimeout <= 0) {
            return;
        }
    ...
    private static class RetryHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (mService.get().mRetryIn > 0) {
                ...
                new Handler(Looper.getMainLooper()).post(() -> {
                    if (mService.get().callbackList.getRegisteredCallbackCount() > 0) {
                        int callbacks = mService.get().callbackList.beginBroadcast();
                        for (int b = 0; b <= callbacks; b++) {
                            try {
                                mService.get().callbackList.getBroadcastItem(b).stateChanged(mService.get().convertState(mService.get().getState()));
                            } catch (Exception e) {
                                if (BuildConfig.DEBUG) {
                                    Log.e("State", "$exception: " + e);
                                }
                        ...
                        mService.get().callbackList.finishBroadcast();
                    }
                });
    ...
    private ConnectionState convertState(State state) {
